...
#define SBI_EXT_VENDOR_SG2042_SET_PMU            0x09000001

#define THEAD_PMU_PDEV_NAME	"thead_pmu"

#define THEAD_IRQ_PMU		17
...
static int thead_pmu_request_irq(irq_handler_t handler)
{
	...
	thead_pmu.irq = irq_create_mapping(domain, THEAD_IRQ_PMU);
	if (!thead_pmu.irq) {
		pr_err("Failed to map PMU interrupt for node\n");
		return -ENODEV;
	}

	ret = request_percpu_irq(thead_pmu.irq, handler, "thead_pmu", &thead_pmu);
	if (ret) {
		pr_err("Unable to request IRQ%d for T-HEAD PMU counters\n",
		       thead_pmu.irq);
		return ret;
	}
...
{
	free_percpu_irq(thead_pmu.irq, this_cpu_ptr(thead_pmu.hw_events));
}
...
static int thead_pmu_device_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		pr_notice("Failed to probe T-HEAD PMU!\n");
		return ret;
	}
	...
	if (ret) {
		...
		pr_notice("T-HEAD PMU request irq fail!\n");
	}

	ret = cpuhp_setup_state(CPUHP_AP_PERF_ONLINE + 1, "perf t-head:online",
	...
	ret = perf_pmu_register(&thead_pmu.pmu, "thead_pmu", PERF_TYPE_RAW);
	if (ret) {
		thead_pmu_free_irq();
		free_percpu(thead_pmu.hw_events);
	}

	pr_notice("T-HEAD PMU probed\n");
...
static struct platform_driver thead_pmu_driver = {
	.driver = {
		.name = THEAD_PMU_PDEV_NAME,
	},
	.probe = thead_pmu_device_probe,
};

int __init thead_pmu_init(void)
{
	int ret;
	struct platform_device *pdev;

	ret = platform_driver_register(&thead_pmu_driver);
	if (ret)
		return ret;

	pdev = platform_device_register_simple(THEAD_PMU_PDEV_NAME, -1, NULL, 0);
	if (IS_ERR(pdev)) {
		platform_driver_unregister(&thead_pmu_driver);
		return PTR_ERR(pdev);
	}

	return ret;
}
device_initcall(thead_pmu_init);
