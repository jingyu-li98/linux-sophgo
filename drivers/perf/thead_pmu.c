...
#define THEAD_PMU_PDEV_NAME "t-head_pmu"
...
static int riscv_pmu_request_irq(irq_handler_t handler)
{
	...
	riscv_pmu_irq = irq_create_mapping(domain, THEAD_IRQ_PMU);
	if (!riscv_pmu_irq) {
	...
	ret = request_percpu_irq(riscv_pmu_irq, handler, "t-head-pmu", &riscv_pmu);
	if (ret) {
...
{
	sbi_ecall(SBI_EXT_VENDOR_SG2042_SET_PMU, 0, 1, 0, 0, 0, 0, 0);
...
static int riscv_pmu_device_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		...
		pr_notice("PMU request irq fail!\n");
	}
	...
	ret = perf_pmu_register(&riscv_pmu.pmu, "t-head_pmu", PERF_TYPE_RAW);
	if (ret) {
		riscv_pmu_free_irq();
		free_percpu(riscv_pmu.hw_events);
	}
...
{
	return riscv_pmu_device_probe(pdev);
}
...
int __init riscv_pmu_init(void)
{
	int ret;
...
device_initcall(riscv_pmu_init);
